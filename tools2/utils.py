...
def  metric(pred_kp,ground_kp,ground_lines,image_path,relaxed = True):
    ...
    e = 1e-10
    recall_oks = (tp_oks+e)/(tp_oks+fn_oks + e)
    precision_oks = (e+tp_oks)/(tp_oks+fp_oks+e)
    F1_oks = 2*tp_oks/(2*tp_oks + fp_oks + fn_oks+e)
...
def get_keypoints(model,image_path,input_size,CUDA_ = "cuda"):
