...
class Point_Head(nn.Module):
    def __init__(self, channel=8,num_classes=3, kernel_size=3, stride=1,bn_momentum=0.1):
        super(Point_Head, self).__init__()
        #-----------------------------------------------------------------#
...
class Header(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True) -> None:
        super().__init__()
        self.conv = Conv(c1,3,k,s)
        self.conv2 = Conv(c1, c2,k,s)
    def forward(self, x):
        return self.conv(x).softmax(dim=1), self.conv2(x)
...
class Detect(nn.Module):
    ...
    def _make_grid(nx=20, ny=20):
        ...
        return torch.stack((xv, yv), 2).view((1, 1, ny, nx, 2)).float().cuda()


class LineSegHead(nn.Module):
    stride = None  # strides computed during build
