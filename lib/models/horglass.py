...
class CenterNet_HourglassNet_Text(nn.Module):
    def __init__(self, nclass, pretrained=False, num_stacks=3, n=5):
        super(CenterNet_HourglassNet_Text, self).__init__()
        if pretrained:
            raise ValueError("HourglassNet has no pretrained model")
        cnv_dim=128
        dims=[128, 128, 192, 192, 192, 256] 
...
class CenterNet_HourglassNet_Line_KPS(nn.Module):

    """
    hourglass 预测：关键点，
    """
    def __init__(self, nclass, pretrained=False, num_stacks=3, n=5):
        super(CenterNet_HourglassNet_Line_KPS, self).__init__()
        if pretrained:
            raise ValueError("HourglassNet has no pretrained model")
        cnv_dim=128
        dims=[128, 128, 192, 192, 192, 256] 
        ...
        for head in heads.keys():
            if 'hm' in head:
                ...
                module = nn.ModuleList([
                    nn.Sequential(
                        conv2d(3, cnv_dim, curr_dim*2, with_bn=False),
                        nn.Upsample(scale_factor=2),
                        conv2d(3, curr_dim*2, curr_dim, with_bn=False),
                        nn.Upsample(scale_factor=2),
                        nn.Conv2d(curr_dim, heads[head], (1, 1))
                    )  for _ in range(num_stacks)
                ])
...
class HourglassNet(nn.Module):
    def __init__(self, nlayers, nclass,):
        ...
        #feature extraction
        self.layer1 = hourglass_block(nclass,128, 128)
    ...
    def forward(self, inputs):
        ...
        for i in range(self.nlayers):
            ...
            for key in result1.keys():
                if key == 'hm':
                    if self.nclass==1:
                        result1[key] =  result1[key].sigmoid()
                    else:
                         result1[key] =  result1[key].softmax(dim=1)
...
if __name__=='__main__':
    ...
    model = CenterNet_HourglassNet(3, pretrained = False)
