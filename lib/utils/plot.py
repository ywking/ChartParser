...
def show_seg_result(img, result, index, epoch, save_dir=None, is_ll=False,palette=None,is_demo=False,is_gt=False, val=False):
    # img = mmcv.imread(img)
    # img = img.copy()
    # seg = result[0]
    if palette is None:
        ...
        if not is_gt:
            if not is_ll:
                ...
                if val:
                    ...
                    cv2.imwrite(save_dir+"{}.png".format(index), result*255)
                cv2.imwrite(save_dir+"/batch_{}_{}_label_viz.png".format(epoch,index), img)
        else:
            if not is_ll:
