...
def random_perspective(combination, targets=(),points=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    if npoints:
         # warp points
        t = points[:,[0,1,2,1,2]]
        xy2 = np.ones((npoints * 4, 3))
        xy2[:, :2] = points[:, [1, 2, 1, 2, 1, 2, 1, 2]].reshape(npoints * 4, 2)  # x1y1, x2y2, x1y2, x2y1
        xy2 = xy2 @ M.T  # transform
        if perspective:
            xy2 = (xy2[:, :2] / xy2[:, 2:3]).reshape(n, 8)  # rescale
        else:  # affine
            xy2 = xy2[:, :2].reshape(npoints, 8)

        # create new boxes
        x2 = xy2[:, [0, 2, 4, 6]]
        y2 = xy2[:, [1, 3, 5, 7]]
        xy2 = np.concatenate((x2.min(1), y2.min(1), x2.max(1), y2.max(1))).reshape(4, npoints).T
        # print(width,height)
        # filter candidates
        i = _point_candiadtes(width, height,xy2.T )
        t = t[i]
        t[:, 1:3] = xy2[i][:,:2]
    combination = (img, line)
    return combination, targets,t[:,:3]
...
def _point_candiadtes(w,h,points):
    return (0<points[0])&(points[0]<w) & (0<points[1])&(points[1]<h)