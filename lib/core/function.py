...
def train(cfg, train_loader, model, criterion, optimizer, scaler, epoch, num_batch, num_warmup,
    ...
    for i, (input, target, paths, shapes) in enumerate(train_loader):
        ...
        if not cfg.DEBUG:
            ...
            assign_target = []
            for tgt in target:
                assign_target.append(tgt.to(device, non_blocking=True))
        ...
        # with amp.scale_loss(total_loss, optimizer) as scaled_losses:
        #     scaled_losses.backward()
        # scaler.scale(total_loss).backward()
        # scaler.step(optimizer)
        # scaler.update()
        # # print(total_loss, '训练到此')
        total_loss.backward()
        optimizer.step()
        ...
        if rank in [-1, 0]:
            ...
            if i % cfg.PRINT_FREQ == 0 and rank in [-1, 0]:
                msg = 'Epoch: [{0}][{1}/{2}]\t' \
...
def validate(epoch,config, val_loader, val_dataset, model, criterion, output_dir,
    ...
    for batch_i, (img, target, paths, shapes) in tqdm(enumerate(val_loader), total=len(val_loader)):
        ...
        with torch.no_grad():
            ...
            # _,da_predict=torch.max(ll_seg_out, 1)
            ...
            if config.TEST.PLOTS:
                if batch_i == 0:
                    for i in range(test_batch_size):
                        ...
                        # 驾驶区域分割
                        # da_seg_mask = ll_seg_out[i][:, pad_h:height-pad_h, pad_w:width-pad_w].unsqueeze(0)
    ...
    if len(stats):
        ...
        if (verbose or (nc <= 20 and not training)) and nc > 1 and len(stats) and rank in [-1,0]:
            for i, c in enumerate(ap_class):
                print(pf % (names[c], seen, nt[c], p[i], r[i], ap50[i], ap[i]))

        # Print speeds
        t = tuple(x / seen * 1E3 for x in (t_inf, t_nms, t_inf + t_nms)) + (imgsz, imgsz, batch_size)  # tuple
        if not training and rank in [-1,0]:
    ...
    return da_segment_result, ll_segment_result, detect_result, losses.avg, maps, t, loss_all
