...
def attentionloss(attentions):
    real_batch_size = attentions[0].shape[0]
    m = nn.Softmax(dim=0)
    attention_loss = 0
    for j in range(1,len(attentions)):
        for i in range(1,real_batch_size):
            target = torch.sum((attentions[j][i].data)**2, dim=0).view(-1) # 按通道求和
            #target = target/torch.max(target)
            target = m(target)

            s = torch.sum(attentions[j-1][i]**2, dim=0).view(-1)
...
class MultiHeadLoss(nn.Module):
    ...
    def _forward_impl(self, predictions, targets, shapes, model):
        ...
        for hg in predictions[1]:
            attentions.append(hg['attention'])
            c_loss          += self.mse(hg['hm'].permute(0,2,3,1), targets[1])
            # wh_loss         = 0.1 * reg_l1_loss(wh, batch_whs, batch_reg_masks)
            off_loss        += reg_l1_loss(hg['reg'], targets[2], targets[3])
        attloss = attentionloss(attentions)
        # 曲线分割Loss
        drive_area_seg_predicts = predictions[-1]
        drive_area_seg_targets = targets[-1]
        lseg_ll = BCEseg(drive_area_seg_predicts, drive_area_seg_targets)
        ...
        detloss = lbox + lobj + lcls
        pointloss =  c_loss+off_loss + 0.1*attloss
        lineloss = lseg_ll + liou_ll
        # loss = lbox + lobj + lcls + lseg_da + lseg_ll + liou_ll
        # var_loss, dist_loss, reg_loss = self.embedingloss(predictions[-1], targets[-1])
        loss =  detloss + pointloss + lineloss  
        # loss = lseg
        # return loss * bs, torch.cat((lbox, lobj, lcls, loss)).detach()
        return loss, (lbox.item(), lobj.item(), lcls.item(), lseg_ll.item(), c_loss.item(), loss.item())
...
def get_kp_seg_det_dill_loss(cfg, device):
    """
    get MultiHeadLoss
    包括 关键点、曲线分割、文本检测、知识蒸馏损失
    Inputs:
...
if __name__=='__main__':
    a = torch.rand(8,128,64,64)
    att = [torch.rand(8,128,64,64),torch.rand(8,128,64,64),torch.rand(8,128,64,64),torch.rand(8,128,64,64)]
    print(attentionloss(att))