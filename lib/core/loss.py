...
class MultiHeadLoss(nn.Module):
    ...
    def _forward_impl(self, predictions, targets, shapes, model):
        ...
        hm_loss = 0.4*( c_loss + off_loss)
        
        if cfg.TRAIN.DET_ONLY or cfg.TRAIN.ENC_DET_ONLY:
            lseg_da = 0 * lseg_da
            c_loss = 0 * c_loss
            off_loss = 0 * off_loss
        if cfg.TRAIN.SEG_DET :
            # lseg_da = 0 * lseg_da
            c_loss = 0 * c_loss
            off_loss = 0 * off_loss
            # lseg_ll = 0 * lseg_ll
            # liou_ll = 0 * liou_ll
        if  cfg.TRAIN.ENC_KP_ONLY:
            lcls = 0 * lcls
            lobj = 0 * lobj
            lbox = 0 * lbox
            lseg_da = 0 * lseg_da
        ...
        if cfg.TRAIN.DRIVABLE_ONLY:  # 关键点
            lcls = 0 * lcls
            lobj = 0 * lobj
            lbox = 0 * lbox
            # lseg_ll = 0 * lseg_ll
            # liou_ll = 0 * liou_ll
        # off_loss = 0
        # loss = lbox + lobj + lcls + lseg_da   +hm_loss
        # print('目标检测损失和分割损失：', lseg_da , liou_ll,'热图损失', c_loss ,off_loss)
...
def get_kp_line_loss(cfg, device):
    """
    计算 关键点检测和曲线分割损失
    # """
    # BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([cfg.LOSS.CLS_POS_WEIGHT])).to(device)
...
class MultiHeadLoss2(nn.Module):
    ...
    def _forward_impl(self, predictions, targets, shapes, model):
        ...
        # loss =  lbox + lobj + lcls + c_loss + lseg_da + off_loss
        loss = c_loss  + 0.2*off_loss + lseg_da
        # loss = lseg
        # return loss * bs, torch.cat((lbox, lobj, lcls, loss)).detach()
        return loss, ( c_loss.item(), loss.item())
