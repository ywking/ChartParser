...
from get_chartboxwh import get_wh,get_wh_txt
...
def main(img_size=512, n=18, thr=0.25, gen=1000):
    ...
    for i in range(18,19):
        ...
        wh = np.array(boxes_wh)
        # print(wh)
        i = (wh < 3.0).any(1).sum()
        wh = wh[(wh >= 2.0).any(1)]  # 只保留wh都大于等于2个像素的box
        # Kmeans calculation
        # print(f'Running kmeans for {n} anchors on {len(wh)} points...')
        # s = wh.std(0)  # sigmas for whitening
        k, dist = kmeans(wh, n, iter=100)  # points, mean distance
